...
void Predictor::Build(const std::shared_ptr<cpp::ProgramDesc> &program_desc,
  ...
  bool hasIntelFpgaTarget = false; // the variable for judge the kIntelFPGA place, added by hc 20220412
  for (auto &valid_place : valid_places) {
    if (valid_place.target == TARGET(kOpenCL)) continue;
    if (valid_place.target == TARGET(kIntelFPGA)) hasIntelFpgaTarget = true; // added by hc 20220412
  ...
  // insert the kIntelFpga place to inner_places, added by hc 20220412
