...
// end commented
#include "lite/kernels/intel_fpga/conv_intelfpga.h" // added by hc 20220416
...
// added by hc 20220416
typedef paddle::lite::kernels::intel_fpga::ConvCompute<PRECISION(kInt8),
                                                       PRECISION(kInt8)>
    ConvInt8_Int8;
// end added
...
// added by hc 20220416
REGISTER_LITE_KERNEL(conv2d, kIntelFPGA, kInt8, kNCHW, ConvInt8_Int8, int8_out)
    ...
    .BindInput("Filter", {LiteType::GetTensorTy(TARGET(kARM), PRECISION(kInt8))})
    .BindOutput("Output", {LiteType::GetTensorTy(TARGET(kARM), PRECISION(kInt8))})
    .BindPaddleOpVersion("conv2d", 1)
    .Finalize();

REGISTER_LITE_KERNEL(depthwise_conv2d, kIntelFPGA, kInt8, kNCHW, ConvInt8_Int8, int8_out)
    .BindInput("Input", {LiteType::GetTensorTy(TARGET(kARM), PRECISION(kInt8))})
    .BindInput("Bias", {LiteType::GetTensorTy(TARGET(kARM), PRECISION(kFloat))})
    .BindInput("Filter", {LiteType::GetTensorTy(TARGET(kARM), PRECISION(kInt8))})
    .BindOutput("Output", {LiteType::GetTensorTy(TARGET(kARM), PRECISION(kInt8))})
